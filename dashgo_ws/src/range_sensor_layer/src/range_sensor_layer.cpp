...
{
  if (range_message.range < range_message.min_range && range_message.range != 0){
  ...
  if (range_message.range == 0 && clear_on_max_reading_){
...
void RangeSensorLayer::updateCostmap(sensor_msgs::Range& range_message, bool clear_sensor_cone)
{
  ...
  double ox = out.point.x, oy = out.point.y; // ox表示sonar在global_frame_下的坐标
  ...
  double tx = out.point.x, ty = out.point.y;  // tx表示障碍点在global_frame_下的坐标
  ...
  // 从世界坐标（ox, oy）向地图坐标(Ox, Oy)转换,地图没有边界
  ...
  unsigned int aa, ab;
  // 从世界坐标（wx, wy）向地图坐标(mx, my)转换,不一定会转换成功，地图坐标系小于世界坐标系，如果转换成功（合法界限），则为True；否则为false
  ...
  //std::cout<<"bx1 = "<<bx1<<"  by1 = "<<by1<<std::endl;
  // std::cout<<"bx0 = "<<bx0<<std::endl;

  // 上面确定了更新扇形区域的大体范围，下面确定更新哪个栅格
  ...
  for (unsigned int x = bx0; x <= (unsigned int)bx1; x++)
  {
    for (unsigned int y = by0; y <= (unsigned int)by1; y++)
    {
      ...
      if (update_xy_cell)
      {
        ...
        // ox oy 是传感器位置坐标 theta是传感器与原点连线和x轴夹角 wx wy 是每个栅格点的坐标 clear_sensor_cone是是否清除Inf点
...
void RangeSensorLayer::update_cell(double ox, double oy, double ot, double r, double nx, double ny, bool clear)
{
  ...
  if (worldToMap(nx, ny, x, y))
  {
    ...
    // phi是costmap中某个栅格到传感器的距离
    double phi = sqrt(dx * dx + dy * dy);
    double sensor = 0.0;
    if (!clear)
      // r是传感器测距结果，phi是costmap中某个栅格到传感器的距离，theta是传感器朝向夹角
      sensor = sensor_model(r, phi, theta);
    // getCost(mx,my)获取一个单元（mx, my）,返回这个单元的代价值
